...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps) {
    ...
    T result = start;
    // Create a variable for the maximum numerical limit of the integer (or other data type)
    T maxLimit = std::numeric_limits<T>::max();
    // Create a variable to return to test_overflow to inform when add number failed.
    T errorWarning = std::numeric_limits<T>::min();
    // Used this as reference to understand and use for my min and max limits
    // since the limits include was given in this code, I took that as a hint.
    // https://en.cppreference.com/w/cpp/types/numeric_limits/min 
    // https://en.cppreference.com/w/cpp/types/numeric_limits/max

    for (unsigned long int i = 0; i < steps; ++i) {
        // This if statement will check and trigger if an overflow has occured
        // and will prevent the result from incrementing further.
        if (maxLimit - increment < result) {
            return errorWarning;
        }
        result += increment;
    }
    // If the increment was succseful, and no overflow occured, the result is returned normally.
    return result;
}
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps) {
    ...
    T result = start;
    // Create a variable to set 0 to be the minimum a number can go.
    T minLimit = 0; 
    // Create a variable to return to test_underflow to inform when subtract number failed.
    // Similar to add_numbers, but reversing it for the error to be the max instead of the min.
    T errorWarning = std::numeric_limits<T>::max();

    for (unsigned long int i = 0; i < steps; ++i) {
        // This if statement will check and trigger if an overflow (underflow) has occured
        // and will prevent the result from decrementing further.
        if (minLimit + decrement > result) {
            return errorWarning;
        }
        result -= decrement;
    }
    // If the decrement was succseful, and no overflow (underflow) occured, the result is returned.
    return result;
}
...
void test_overflow()
{
    ...
    // Modified the adding numbers with overflow, because this is expected. (Above unchanged)
    // If the result received is equal to the minimum, an oferflow occured
    // And therefore we get an overflow occured message.
    if (result == std::numeric_limits<T>::min()) {
        std::cout << "Numerical limit overflow occured." << std::endl;
    }
    // Otherwise, the same result print message of without overflow is used.
    else {
        std::cout << +result << std::endl;
    }
}
...
void test_underflow()
{
    ...
    // Modified the subracting numbers with overflow, because this is expected. (Above unchanged)
    // If the result received is equal to the maximum, an overflow occured
    // And therefore we get an overflow occurred message
    if (result == std::numeric_limits<T>::max()) {
        std::cout << "Numerical limit underflow occured." << ::std::endl;
    }
    // Otherwise, the same result print message of without overflow is used.
    else {
        std::cout << +result << std::endl;
